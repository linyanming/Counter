...
void LCD_WriteNum(u8 bNum,u8 pos,bool dot)
{
	switch(pos)
	{
		case DSP1:
			...
			LCD->RAM[LCD_RAMRegister_1] |= (u8)((LCM_Num[bNum] & 0x03));
			//COM1  
			LCD->RAM[LCD_RAMRegister_4] &= (~0x30);
			LCD->RAM[LCD_RAMRegister_4] |= (u8)((LCM_Num[bNum] & 0x0C) >> 2);
			//COM2  
			LCD->RAM[LCD_RAMRegister_8] &= (~0x03);
			LCD->RAM[LCD_RAMRegister_8] |= (u8)((LCM_Num[bNum] & 0x30) >> 4);
			//COM3  
			LCD->RAM[LCD_RAMRegister_11] &= (~0x30);
			LCD->RAM[LCD_RAMRegister_11] |= (u8)((LCM_Num[bNum] & 0xC0) >> 6);
			break;
		case DSP2:
			//COM0      
			LCD->RAM[LCD_RAMRegister_1] &= (~0x03);
